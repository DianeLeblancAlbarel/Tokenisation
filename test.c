...
double tablefealingrate (){
    ...
    TOKEN_T token[1];
    int success=1;
    unsigned long long numberRow = 0;
    int numberTry=0;
    while (success == 1 && numberRow<NUM_ROWS ){
       success = tokenization(table,(CARD_T) 4837562834756767+numberRow ,(USES_T) 1 ,(TIME_T) temp+1000, pk, token, key, iv,&numberTry);
        numberRow+=1;
    }
    free(table);
    if(numberRow==NUM_ROWS)
        ...
        return ((double)numberRow/NUM_ROWS)*100;
}

 void tryPerTimeframe (int *tryPerTimeFrame,int *lastRow,double *fealingRate){
    RAND_bytes(key, 32);
    RAND_bytes(iv,16);
    unsigned long long size = (unsigned long )NUM_ROWS * (unsigned long long)ROW_BYTES;
    uint8_t * table=calloc(size,sizeof(uint8_t));
    ...
    if (rowNumber==NUM_ROWS-1){
        ...
        *lastRow=rowNumber-1;
    *fealingRate=((double)*lastRow/(NUM_ROWS-1))*100;
}
...
tryPerTimeframe(tryPerTimeFrame,&lastRow,&fealing);
minmaxValue(tryPerTimeFrame,lastRow,&min,&max);
printf("table fealing rate for %d rows : %f%%\n",NUM_ROWS,fealing);

if(fealing<100)
    printf("try at the first failure : %d\n",*(tryPerTimeFrame+lastRow));
printf("try at the last row : %d\nmaximum try %d\nminimum try %d\n",*(tryPerTimeFrame+lastRow-1),max,min);
